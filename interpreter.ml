 ...
 *)
 ...
 | AST_write of (ast_e)
 | AST_assign of (string * ast_e * row_col * row_col)    (* id location, := location *)
 ...
 and ast_ize_stmt_list (sl:parse_tree) : ast_sl =          
   match sl with
   | PT_nt("SL", _, []) -> [] (*handles epsilon case*)
   | PT_nt("SL", _, [s; slnew]) -> ast_ize_stmt s @ ast_ize_stmt_list slnew
   (*| PT_nt("SL", _, stmts) -> List.flatten (List.map ast_ize_stmt stmts)*)  (*borrowed from ocaml help docs*)   (* | PT_nt("SL", _, stmts) -> ast_ize_stmt stmts  *)
   | _ -> raise (Failure "malformed parse tree in ast_ize_stmt_list")
 
 and ast_ize_stmt (s:parse_tree) : ast_sl =
   match s with
   | PT_nt("S", _, [PT_id(lhs, vloc); PT_term(":=", aloc); expr]) -> [AST_assign(lhs, (ast_ize_expr expr), vloc, aloc)]      (* vloc is the place to complain about undeclared lhs; aloc (:= sign) is the place to complain about a type clash *)
   | PT_nt("S", _, [PT_term("int", iloc); PT_id(lhs, vloc); PT_term(":=", aloc); expr]) -> [AST_i_dec(lhs, iloc); AST_assign(lhs, (ast_ize_expr expr), vloc, aloc)]  (* is this paring correct to get the declaration and assignment of id... if so we do the same thing for real right*)
   | PT_nt("S", _, [PT_term("real", rloc); PT_id(lhs, vloc); PT_term(":=", aloc); expr]) -> [AST_r_dec(lhs, rloc); AST_assign(lhs, (ast_ize_expr expr), vloc, aloc)]   (*is it PT_real or PT_term??? for both-^*)
   (*| PT_nt("S", _, [PT_term("read", _); PT_nt("TP", _, [PT_term (tp, _)]); PT_id(id, vloc)]) -> [AST_read(id, vloc)]*) (*what do we do with the tp? does it go in the AST_read?        error above: This pattern matches values of type 'a * 'b but a pattern was expected which matches values of type string * row_col * parse_tree list*)
   | PT_nt("S", _, [PT_term("read", _); PT_nt("TP", _, [PT_term (tp, _)]); PT_id(lhs, vloc)]) ->
    begin
        match tp with
        | "int" -> [AST_i_dec(lhs, vloc); AST_read(lhs, vloc)]
        | "real" -> [AST_r_dec(lhs, vloc); AST_read(lhs, vloc)]
        | _ -> raise (Failure "Invalid type in read declaration")
    end
   
   | PT_nt("S", _, [PT_term("write", _); expr]) -> [AST_write(ast_ize_expr expr)]
   | PT_nt("S", _, [PT_term("if", _); cond; sl; PT_term("fi", _)]) -> [AST_if(ast_ize_cond cond, ast_ize_stmt_list sl)]
   | PT_nt("S", _, [PT_term("do", _); sl; PT_term("od", _)]) -> [AST_do(ast_ize_stmt_list sl)]
   | PT_nt("S", _, [PT_term("check", cloc); cond]) -> [AST_check((ast_ize_cond cond), cloc)]   (*is it correct to have cloc here ? *)
   | _ -> raise (Failure "malformed parse tree in ast_ize_stmt")
 

 and ast_ize_expr (e:parse_tree) : ast_e =   (* E, T, or F *)
   match e with
   | PT_nt("E", _, [term; tail]) -> ast_ize_expr_tail (ast_ize_expr term) tail
   | PT_nt("T", _, [term; tail]) -> ast_ize_expr_tail (ast_ize_expr term) tail
 ...
 and ast_ize_expr_tail (lo:ast_e) (tail:parse_tree) : ast_e =   (* TT or FT *)
   ...
   match tail with
   | PT_nt ("TT", _, []) -> lo      (*for handling epsilon case...... do we return lo here? or [] *)   (*WHERE DO WE RETURN LO?????*)
   | PT_nt ("FT", _, []) -> lo    (*for handling epsilon case*)
   | PT_nt("TT", _, [PT_nt("AO", _, [PT_term(op, oloc)]); term; tail]) -> ast_ize_expr_tail (AST_binop(op, lo, ast_ize_expr term, oloc)) tail         (* error above: This pattern matches values of type 'a * 'b but a pattern was expected which matches values of type string * row_col * parse_tree list *)
   | PT_nt("FT", _, [PT_nt("MO", _, [PT_term(op, oloc)]); factor; tail]) -> ast_ize_expr_tail (AST_binop(op, lo, ast_ize_expr factor, oloc)) tail
   (*| PT_nt("FT", _, [PT_nt("MO", _); PT_term(op, oloc); factor; tail]) -> ast_ize_expr_tail (AST_binop(op, lo, ast_ize_expr factor, oloc)) tail      error above: This pattern matches values of type 'a * 'b but a pattern was expected which matches values of type string * row_col * parse_tree list *)
   (* do we include this case "| _ -> lo" ........ as we may need to return lo as we need to return an ast_e eventually*)
   | _ -> raise (Failure "malformed parse tree in ast_ize_expr_tail")
 
 and ast_ize_cond (c:parse_tree) : ast_c =
   match c with
   PT_nt("C", _, [expr1; PT_nt("RO", _, [PT_term(op, oloc)]); expr2]) -> (op, ast_ize_expr expr1, ast_ize_expr expr2, oloc) (*error above: This pattern matches values of type 'a * 'b but a pattern was expected which matches values of type string * row_col * parse_tree list <<<<<<<- Because of old line ->>>>>>    (* | PT_nt("C", _, [expr; PT_nt("RO", _); PT_term(op, oloc); expr2]) -> (op, ast_ize_expr expr, ast_ize_expr expr2, oloc)  *) *) 
